...
部分电视彻底断电并重新接通电源后，要等用户按遥控器才会连接 Wi-Fi；此时 Keeper 的 90 秒有限重试可能已经结束。m-ui 只在电视的 `6095` 接口连续离线至少 120 秒后重新上线、且 ADB 仍不可用时，通过 `6095` 单次启动 Keeper。短暂静默重启不会达到该门槛，单次请求失败后也不会在同一在线会话中循环重试。
