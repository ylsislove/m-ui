...
选择 `4. 启动 m-ui`。第一次连接时，电视应显示 ADB 调试授权弹窗，设备名称通常为 `m-ui@router`。选择始终允许后，m-ui 会在下一轮自动连接。
...
如果需要同时修改电视 IP，请直接编辑 `/etc/m-ui/config` 中的 `TV_IP`；环境变量只在第一次创建配置时生效。
